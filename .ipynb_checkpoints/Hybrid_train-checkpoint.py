...
try:
    ...
    pass
os.makedirs(f'{opt.out}/logs/', exist_ok=True)
os.makedirs(f'{opt.out}/models/', exist_ok=True)
# Setup logging
current_time = datetime.now().strftime('%Y%m%d-%H%M%S')
log_filename = f'{opt.out}/logs/training_log_{current_time}.log'
...
# Model setup
classifier = HybridDenseCls(k=num_classes, feature_transform=opt.feature_transform)
best_classifier = HybridDenseCls(k=num_classes, feature_transform=opt.feature_transform)
if opt.model != '':
